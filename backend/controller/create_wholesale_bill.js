    ...
    try {
        ...
        for (const item of processedItems) {
            ...
            await client.query(
                ...
                [item.product_id, item.quantity, billId, userId]
            );
        }
    ...
    try {
        ...
        const itemsQuery = `
            SELECT 
                wbi.*,
                p.product_name,
                COALESCE(un.unit_name, 'pcs') AS unit_name
            FROM wholesale_bill_items wbi
            JOIN products p ON wbi.product_id = p.product_id
            LEFT JOIN units un ON p.unit_id = un.unit_id          -- ← FIXED: JOIN via products.unit_id
...
exports.generatePdf = async (req, res) => {
    ...
    try {
        ...
        // ── FIXED: JOIN units through products.unit_id ──────────────────────
        const itemsQuery = `
            SELECT 
                wbi.*,
                p.product_name,
            ...
            FROM wholesale_bill_items wbi
            JOIN products p  ON wbi.product_id = p.product_id
            LEFT JOIN units un ON p.unit_id = un.unit_id          -- products → units
            WHERE wbi.wholesale_bill_id = $1
        `;
        ...
        pdfGenerator.buildPDF(
            (chunk) => { if (!res.writableEnded && !res.closed) res.write(chunk); },
