...
router.get('/', auth,auth.authorize(allowed), viewBillController.getRetailBills);

// 2. Record Payment for a due bill
// POST /api/view-retail-bill/pay
router.post('/pay', auth,auth.authorize(allowed), viewBillController.recordPayment);

// 3. Download PDF
// GET /api/view-retail-bill/:id/pdf
router.get('/:id/pdf', auth,auth.authorize(allowed), viewBillController.downloadBillPdf);
