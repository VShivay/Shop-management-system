...
function buildPDF(dataCallback, endCallback, billData) {
    // A4 Size, margins
    ...
    generateDetailsSection(doc, billData);

    // -- 3. Item Table --
    // Returns the Y position where the table ended
    let finalY = generateInvoiceTable(doc, billData);

    // -- 4. Payment History (NEW) --
    // We pass finalY to start drawing from there, and get a new Y back
    finalY = generatePaymentHistory(doc, billData, finalY);

    // -- 5. Financial Summary & Footer --
    generateFooter(doc, billData, finalY);
...
function generateHeader(doc) {
    doc
        ...
        .font('Helvetica-Bold')
        .fontSize(24)
        .text('RETAIL INVOICE', 40, 40, { align: 'left' });
    ...
    doc
        ...
        .font('Helvetica')
        .text('123 Main Street, Complex Road', 200, 55, { align: 'right' })
...
function generateDetailsSection(doc, bill) {
    ...
    if (bill.customer_phone) {
        doc.text(`Phone: ${bill.customer_phone}`, 50, currentY);
        currentY += 15;
    }
    if (bill.customer_address) {
        doc.text(bill.customer_address, 50, currentY, { width: 230 });
    }
    ...
    printMeta('Pay Mode:', bill.payment_method_name || 'Cash');
    
    doc.text('Status:', labelX, rightY);
    const statusColor = bill.payment_status === 'paid' ? '#166534' : 
                        bill.payment_status === 'unpaid' ? '#991b1b' : '#9a3412';
    
    doc.fillColor(statusColor).font('Helvetica-Bold')
       .text(bill.payment_status.toUpperCase(), valueX, rightY);
...
function generateInvoiceTable(doc, bill) {
    ...
    for (i = 0; i < bill.items.length; i++) {
        ...
        doc.text(item.unit_price, xPrice, y, { width: 70, align: 'right' });
        doc.text(item.quantity, xQty, y, { width: 60, align: 'right' });
        doc.text(item.total_price, xTotal, y, { width: 60, align: 'right' });
...
// --- NEW FUNCTION: Payment History ---
function generatePaymentHistory(doc, bill, startY) {
    ...
    bill.payment_history.forEach(ph => {
        if (y > 750) {
            doc.addPage({ size: 'A4', margin: 40 });
            y = 50;
        }
        ...
        doc.text(ph.amount_paid, xAmount, y, { align: 'right', width: 60 });
...
function generateFooter(doc, bill, startY) {
    ...
    drawSummaryRow("Subtotal", bill.subtotal);
    drawSummaryRow("Tax", bill.tax_amount);
    drawSummaryRow("Discount", bill.discount_amount);
    
    generateHr(doc, summaryY);
    summaryY += 5;

    drawSummaryRow("Grand Total", `INR ${bill.total_amount}`, true, false, true);
    
    // Total Paid (Includes history + initial)
    drawSummaryRow("Total Paid", `INR ${bill.amount_paid}`, false);

    const due = (Number(bill.total_amount) - Number(bill.amount_paid)).toFixed(2);
    if (Number(due) > 0) {
        drawSummaryRow("Balance Due", `INR ${due}`, true, true);
    }
    ...
    doc.font('Helvetica').fontSize(8).fillColor('#9ca3af')
       .text('Computer-generated invoice.', 40, 780, { align: 'center', width: 515 });
}
