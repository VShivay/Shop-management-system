...
def merge_inventory_transactions(src_cur, tgt_cur, product_map, supplier_map, user_map):
    ...
    src_cur.execute(
        """SELECT product_id, transaction_type, quantity, reference_id, reference_type,
                  supplier_id, performed_by, transaction_date, remarks
           FROM inventory_transactions ORDER BY transaction_date;"""
    )
    inserted = 0
    for row in src_cur.fetchall():
        (src_pid, txn_type, qty, ref_id, ref_type,
        ...
        # Deduplicate: same product + type + qty + date + reference
        tgt_cur.execute(
            """SELECT transaction_id FROM inventory_transactions
               WHERE product_id=%s AND transaction_type=%s AND quantity=%s
                 AND transaction_date=%s AND reference_type IS NOT DISTINCT FROM %s;""",
            (tgt_pid, txn_type, qty, txn_date, ref_type)
        )
        if tgt_cur.fetchone():
            continue
        ...
        tgt_cur.execute(
            ...
            (tgt_pid, txn_type, qty, ref_id, ref_type, tgt_sup, tgt_user, txn_date, remarks)
        )
        inserted += 1
    log.info(f"  [inventory_transactions] Inserted {inserted} new transaction(s).")
